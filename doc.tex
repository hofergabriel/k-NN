...
One processor is assigned to each record or plant in the data set. The root process (process with rank 0) manages communication, divides the data to 
...
Then, process 0 sends a record (4 attributes, class) to a unique processor.
Similarily, process 0 read each record from the file containing the query examples
and stores each record into an array called $query$. 
Process 0 sends the same query to every process with rank $>0$. Then every process 
computes the manhattan distance between their assigned record from the training
set and the query example.
...
The only larger combined task is the root process where 
the other processes send the results from calculating the manhattan distance.
...
of a process tha the record is sent to:
\[
...
For example, iris.data has 150 lines (or records) of data. So, I run the program with 151 processes. This is because the root isn't assigned a record in the data set.
...
\noindent This is the format for how command line parameters read by the program:
\begin{verbatim}
...
a. Load the data
\end{verbatim}
\noindent The name of the data-file is read from argv[1]. records are read from the 
...
\end{verbatim}
qsort is called to sort pairs of data contained in arr. The first element in the pair is the manhattan distance. The second element is the index or label of the record in the data array.
\begin{lstlisting}[frame=single]
...
\end{verbatim}
The only difference between records in the training set and the query set is that each plant in the query set doesn't have a class field.
...
This program ran significancly slower to run in parallel. I'm guessing that this is because all processes were run on my PC which only has a few cores. 
